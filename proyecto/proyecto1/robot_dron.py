...
class RobotDron(RobotBase):
    #Robot dron aéreo.
    #Hereda del RobotBase su movimiento rápido pero la baja capacidad de carga.
    #Debe estar en vuelo para poder moverse y limpiar.
    ...
    def despegar(self):
        # Se activa el modo de vuelo del dron
        print(f"[{self.get_nombre()}] Despegando hasta {self.altura_maxima} metros de altura.")
        self.en_vuelo = True

    def mover(self):
        #Si está en vuelo, se mueve rápido y si no, retorna sin moverse.
    ...
    def limpiar(self):
        #Solo limpia si está en vuelo. Consume 3.0 de batería y recoge entre 0.1 y 0.4 kg de basura
        if self.en_vuelo:
